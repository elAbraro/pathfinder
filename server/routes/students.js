...
router.put('/shortlist/:universityId/status', auth, async (req, res) => {
  try {
    ...
    // Populate student with university details to get university name for notification
    ...
    const shortlistItem = student.shortlistedUniversities.find(
      item => item.university && item.university.toString() === universityId
    );
    ...
    try {
      ...
      await Notification.create({
        ...
        type: 'Info',
        relatedLink: `/application/${shortlistItem.university._id}`
      });
